...
from lib.dataset.dreyeve import DREYEVE,collate_fn_filter
from lib.dataset.dreyeved import DREYEVED,collate_fn_filter_d
...
def main():
    ...
    train_dataset_bdda = DREYEVE(split='train', aug = True)
    valid_dataset_bdda = DREYEVE(split='test', aug = False)
    
   
    train_loader_bdda = torch.utils.data.DataLoader(
        train_dataset_bdda,
        batch_size=cfg.TRAIN.BATCH_SIZE_PER_GPU*len(cfg.GPUS), shuffle=True,
        num_workers=cfg.WORKERS, pin_memory=True,collate_fn=collate_fn_filter)
    ...
    train_dataset_bddad = DREYEVED(split='train')
    valid_dataset_bddad = DREYEVED(split='test')
    
   
    train_loader_bddad = torch.utils.data.DataLoader(
        train_dataset_bddad,
        batch_size=cfg.TRAIN.BATCH_SIZE_PER_GPU*len(cfg.GPUS), shuffle=True,
        num_workers=cfg.WORKERS, pin_memory=True,collate_fn=collate_fn_filter_d)
...
def load_checkpoint_eyemodel(filename='checkpoint_org.pth.tar'):
    filename = os.path.join('./output/dreyeved', filename)
    print(filename)

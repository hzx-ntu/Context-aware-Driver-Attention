...
class Attention(nn.Module):
    def __init__(self, dim, fmap_size, heads = 8, dim_key = 32, dim_value = 64, dropout = 0., dim_out = None, downsample = False):
        ...
        q_range_h = torch.arange(0, fmap_size[0], step = (2 if downsample else 1))
        q_range_w = torch.arange(0, fmap_size[1], step = (2 if downsample else 1))
        k_range_h = torch.arange(fmap_size[0])
        k_range_w = torch.arange(fmap_size[1])

        q_pos = torch.stack(torch.meshgrid(q_range_h, q_range_w), dim = -1)
        k_pos = torch.stack(torch.meshgrid(k_range_h, k_range_w), dim = -1)
...
class Transformer(nn.Module):
    ...
    def forward(self, x):
        for attn, ff in self.layers:
            ...
            x,weights=attn(x) #hzx added
            x=x + attn_res #hzx added
            #x = attn(x) + attn_res #original
            x = ff(x) + x
        return x, weights
